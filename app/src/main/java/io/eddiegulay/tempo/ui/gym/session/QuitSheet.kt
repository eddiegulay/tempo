...
fun QuitSheet(
    ...
    PlayerSheet(
        ...
        PlayerSheetRow(
            ...
            // 「これまでの記録は消えます」 is the whole reason this row is not the accent one, and the
            // armed sentence is assertive on purpose: it is a destructive confirmation, which is the
            // one place in this feature where interrupting the user is right.
            ...
            onClick = {
                when {
                    // Under one completed segment there is nothing to lose, so it deletes immediately.
