...
- **Search (検索)**: live-filtered list of every installed app (work-profile apps included), with a
  scale-up launch animation and a long-press menu (app info / hide / uninstall). With contacts
  access, a name or number shows matching people with Call, Message, and WhatsApp. Without it, the
  same query is handed to Phone, Contacts, WhatsApp, or Google. Tempo does not read the call log.
  The hidden-apps page, the theme toggle and the language picker live in this screen's header.
